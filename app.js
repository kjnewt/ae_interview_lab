(function () {
  ...
  function weakestCompetencies() {
    return rubric.map((item,index) => ({ name: item[0], rating: Number(state.ratings[index]) }))
  ...
  function init() {
    selectMode(state.mode || "panel");
    renderRubric();
    bindEvents();
  }

  if (typeof document !== "undefined") document.addEventListener("DOMContentLoaded", init);
  if (typeof module !== "undefined" && module.exports) module.exports = { questions, rubric, modeInfo, selectQuestions, calculateScore, evaluateReadiness, formatTime, countWords };
})();
